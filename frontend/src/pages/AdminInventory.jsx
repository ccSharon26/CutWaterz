...
export default function AdminInventory({ onLogout }) {
  ...
  const handleAddProduct = async (e) => {
    e.preventDefault();
    if (!name || !price) return alert("Fill in all details!");
    try {
      setLoading(true);
      await addProduct({ name, price: parseFloat(price), stock: parseInt(stock || 0) });
  ...
  return (
    <div className="bg-[#0f0f0f] text-gray-100 min-h-screen pt-20 px-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={onLogout} className="bg-red-600 px-3 py-1 rounded font-semibold hover:bg-red-700">
          Logout
        </button>
      </div>
      ...
      <form onSubmit={handleAddProduct} className="border border-gray-800 p-4 rounded-lg mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          ...
          <input type="number" placeholder="Initial Stock (optional)" value={stock} onChange={(e) => setStock(e.target.value)} className="p-2 rounded bg-[#0f0f0f] border border-gray-700 text-gray-100" />
          <button type="submit" className="bg-amber-500 text-black rounded font-semibold hover:bg-amber-600 transition">{loading ? "Saving..." : "Add Product"}</button>
        </div>
      </form>
